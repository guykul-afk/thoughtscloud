...
function cleanCode(code) {
    return code.split('__BT__').join(BT).split('__DOLLAR__').join('$');
}
...
const buildGraphContext = (knowledgeGraph?: { nodes: any[]; edges: any[] }): string => {
    ...
    const nodesStr = knowledgeGraph.nodes.map(n => \`- __DOLLAR__{n.label} (חשיבות: __DOLLAR__{n.val ? n.val.toFixed(1) : '1.0'})\`).join('\\n');
    const edgesStr = knowledgeGraph.edges.map(e => \`- [__DOLLAR__{e.source}] --(__DOLLAR__{e.relation})--> [__DOLLAR__{e.target}]\`).join('\\n');
    return __BT__
...
const newMajor = cleanCode(`export async function generateMajorInsights(
    allEntries: { transcript: string; timestamp: number }[], 
    apiKey: string,
    currentInsights: string[] = [],
    knowledgeGraph?: { nodes: any[]; edges: any[] }
