...
function cleanCode(code) {
    let cleaned = code.split('__BT__').join(BT).split('__DOLLAR__').join('$');
    return cleaned.split('\r\n').join('\n');
}

// Define buildGraphContext helper code
const buildGraphContextCode = cleanCode(`
...
if (!content.includes('const buildGraphContext =')) {
...
const oldWeeklyBriefing = cleanCode(`export async function generateWeeklyBriefing(allEntries: { transcript: string; timestamp: number }[], apiKey: string): Promise<string> {
...
const newWeeklyBriefing = cleanCode(`export async function generateWeeklyBriefing(
    ...
    const prompt = __BT__
  You are an expert personal growth coach and analyst for "גיא" (Guy).
  Today is: __DOLLAR__{currentDateTime}.
  I am providing you with all of Guy's diary entries from the past week.
  Please provide a deep, high-level "Weekly Insight" (תובנה שבועית) that summarizes the main themes, emotional patterns, and progress Guy has made.
  ...
  *CRITICAL SHADOW WORK REQUIREMENT*: Look for contradictions. What is Guy avoiding? What excuses is he making? Point out any cognitive dissonance or "stories" he tells himself to avoid pain or effort. Be direct but constructive (Devil's Advocate approach).
  ...
  Recent material:
  __DOLLAR__{recentTranscripts}

  __DOLLAR__{graphText}
...
const oldCategorical = cleanCode(`export async function generateCategoricalInsights(allEntries: { transcript: string; timestamp: number }[], apiKey: string): Promise<{ work: string; family: string; personal: string }> {
    const genAI = getGenAI(apiKey);
...
const newCategorical = cleanCode(`export async function generateCategoricalInsights(
    allEntries: { transcript: string; timestamp: number }[],
    apiKey: string,
    knowledgeGraph?: { nodes: any[]; edges: any[] }
...
const oldShadow = cleanCode(`export async function generateShadowQuickAdvices(shadowWorkInsight: string, allEntries: { transcript: string; timestamp: number }[], apiKey: string): Promise<string[]> {
    const genAI = getGenAI(apiKey);
    const model = genAI.getGenerativeModel({ model: activeModelName }, { apiVersion: activeApiVersion as any });
...
const newShadow = cleanCode(`export async function generateShadowQuickAdvices(
...
// 4. generateAdvices
const oldAdvices = cleanCode(`export async function generateAdvices(allEntries: { transcript: string; timestamp: number }[], apiKey: string): Promise<{ work: string; family: string; mental: string }> {
    const genAI = getGenAI(apiKey);
    const model = genAI.getGenerativeModel({ model: activeModelName }, { apiVersion: activeApiVersion as any });
...
const newAdvices = cleanCode(`export async function generateAdvices(
    allEntries: { transcript: string; timestamp: number }[],
    apiKey: string,
    knowledgeGraph?: { nodes: any[]; edges: any[] }
...
const oldThemes = cleanCode(`export async function generateLifeThemesAnalysis(allEntries: { transcript: string; timestamp: number }[], apiKey: string, type: 'weekly' | 'monthly'): Promise<string> {
    const genAI = getGenAI(apiKey);
    const model = genAI.getGenerativeModel({ model: activeModelName }, { apiVersion: activeApiVersion as any });
...
const newThemes = cleanCode(`export async function generateLifeThemesAnalysis(
    allEntries: { transcript: string; timestamp: number }[],
    apiKey: string,
    type: 'weekly' | 'monthly',
    knowledgeGraph?: { nodes: any[]; edges: any[] }
...
const oldGap = cleanCode(`export async function analyzeExecutionGap(allEntries: { transcript: string; tasks?: any[]; timestamp: number }[], apiKey: string): Promise<string> {
    const genAI = getGenAI(apiKey);
    const model = genAI.getGenerativeModel({ model: activeModelName }, { apiVersion: activeApiVersion as any });
...
const newGap = cleanCode(`export async function analyzeExecutionGap(
...
const oldGTD = cleanCode(`export async function generateEmotionalGTDInsight(allEntries: { transcript: string; timestamp: number }[], apiKey: string): Promise<string> {
    const genAI = getGenAI(apiKey);
    const model = genAI.getGenerativeModel({ model: liteModelName }, { apiVersion: activeApiVersion as any });
...
const newGTD = cleanCode(`export async function generateEmotionalGTDInsight(
    allEntries: { transcript: string; timestamp: number }[],
    apiKey: string,
    knowledgeGraph?: { nodes: any[]; edges: any[] }
): Promise<{ insight: string; triples: [string, string, string][] }> {
...
content = content.replace(oldGTD, newGTD);

// 8. generateOperatingManual
const oldManual = cleanCode(`export async function generateOperatingManual(allEntries: { transcript: string; timestamp: number }[], apiKey: string): Promise<string> {
    const genAI = getGenAI(apiKey);
    const model = genAI.getGenerativeModel({ model: activeModelName }, { apiVersion: activeApiVersion as any });
...
const newManual = cleanCode(`export async function generateOperatingManual(
    allEntries: { transcript: string; timestamp: number }[],
    apiKey: string,
    knowledgeGraph?: { nodes: any[]; edges: any[] }
): Promise<{ insight: string; triples: [string, string, string][] }> {
...
const oldMajorCodeInFile = cleanCode(`export async function generateMajorInsights(
    allEntries: { transcript: string; timestamp: number }[], 
    apiKey: string,
...
const newMajorCode = cleanCode(`export async function generateMajorInsights(
    allEntries: { transcript: string; timestamp: number }[], 
    apiKey: string,
    currentInsights: string[] = [],
    knowledgeGraph?: { nodes: any[]; edges: any[] }
  ...
  חומר שבועי:
  __DOLLAR__{weeklyTranscripts || "אין מספיק נתונים מהשבוע."}

  חומר גלובלי (נציגותי):
  __DOLLAR__{globalTranscripts}

  __DOLLAR__{graphText}

  תובנות קיימות (למטרת יציבות):
  __DOLLAR__{currentInsights.length > 0 ? currentInsights.join('\\n') : "אין תובנות קודמות."}
...
content = content.replace(oldMajorCodeInFile, newMajorCode);

// 10. Append generateBiDailyThreads at the end of the file
const biDailyThreadsCode = cleanCode(`
export async function generateBiDailyThreads(
    ...
    const recentEdgesStr = recentEdges.map(e => \`- [__DOLLAR__{e.source}] --(__DOLLAR__{e.relation})--> [__DOLLAR__{e.target}]\`).join('\\n');

    const recentTranscripts = newEntries
        .map(e => \`[Entry Date: __DOLLAR__{new Date(e.timestamp).toLocaleString('he-IL')}]: __DOLLAR__{e.transcript}\`)
        .join('\\n\\n');
  ...
  __DOLLAR__{recentEdgesStr || 'No new graph relationships.'}
...
if (!content.includes('generateBiDailyThreads')) {
  content = content + '\n' + biDailyThreadsCode;
}

fs.writeFileSync(filePath, content, 'utf8');
console.log('Finished normalizing and replacing src/services/ai.ts successfully!');
