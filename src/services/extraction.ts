...
export async function generateClarifyingQuestion(sessionTranscript: string, apiKey: string): Promise<string> {
    ...
    1. המטרה היא לחשוף את הידע הסמוי ולהוסיף מידע קריטי: מה ההקשר החסר? למה גיא חושב כך? מתי הכלל הזה תקף (תנאי סף)?
    2. עליך לנסח שאלת חידוד אחת בלבד. השאלה חייבת להיות קשורה באופן ישיר וספציפי לתוכן הרשומה האחרונה, ולהיות ממוקדת בהוספת מידע חסר או הבהרת המצב.
    3. השאלה תהיה קצרה, נוקבת וישירה (עד 15-20 מילים). ללא נימוסים או הקדמות.
    4. אם הטקסט כבר מכיל מספיק מידע, רציונל עמוק ובהיר, ואין פער לוגי להשלים - חובה עליך להחזיר בדיוק את המילה "DONE".
